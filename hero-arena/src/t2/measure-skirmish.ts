#!/usr/bin/env bun
/**
 * 1v1 heroes + ~10 scripted minions per side — the "common case" for the runtime AI.
 *   bun hero-arena/src/t2/measure-skirmish.ts <preset1> <preset2> [seed]
 */
...
function wrap(_n: string, ctl: HeroController, samples: number[]): HeroController {
  return (ctx) => { const t0 = Date.now(); const a = ctl(ctx); samples.push(Date.now() - t0); return a; };
}

const samples: Record<string, number[]> = { [preset1]: [], [preset2]: [] };
const config: ArenaConfig = {
  seed,
  // Asymmetric: blue has 10 minions, red has none. The realistic single-player case.
...
function stats(arr: number[]) {
  ...
  return { n: arr.length, avg: sum / arr.length, median: sorted[Math.floor(sorted.length / 2)]!,
           p90: sorted[Math.floor(sorted.length * 0.9)]!, max: sorted[sorted.length - 1]! };
}
const minionCount = MINIONS.reduce((n, m) => n + m.count, 0);
console.log(`# 1v1+${minionCount}min: ${preset1} (red) vs ${preset2} (blue), seed ${seed}, ${state.turnNumber} game-turns`);
for (const name of [preset1, preset2]) {
  const s = stats(samples[name]!); if (!s) continue;
