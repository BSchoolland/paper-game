...
const spec = await agent(
  [
    ...
    "- items: an array of EXACTLY 16 items: 11 weapons, 1 shield, 2 accessories, 2 consumables. Each item is { name, type: one of sword/spear/bow/staff/two-handed/shield/accessory/consumable, rarity: common/uncommon/rare/epic/legendary, description: brief (~1 sentence) }. Rarity budget: 4-5 common, 4-5 uncommon, 3-4 rare, 2 epic, 1 legendary. Rarity buys RULES, not stats: a common is a solid tool with one spatial twist; a rare has an internal set-up/payoff synergy; an epic changes a rule of play (teleports, summons, trades places, converts resources); the legendary is a build-around identity the player reroutes their whole kit for. The description should hint at the mechanical identity. Example: { \"name\": \"Spear\", \"type\": \"spear\", \"rarity\": \"common\", \"description\": \"A long polearm with superior reach.\" }",
...
const itemPrompt = [
  ...
  "",
  "REFERENCE DATA TO LOAD FIRST (run these before designing):",
  `  - Item definition formats (copy these shapes exactly): dump the dimension-706 showcase roster — the reference implementation of the rarity ladder across all four types:`,
  `      cd ${ROOT}/server && ${ENV} bun -e 'import { loadItems } from "./src/db.ts"; const r = loadItems(706); for (const k of Object.keys(r)) if (k.includes("vinewalker")||k.includes("riposte")||k.includes("stormcaller")||k.includes("colossus")||k.includes("idol")||k.includes("sapling")) console.log(k, JSON.stringify(r[k], null, 2));'`,
  `  - The authoritative zod schemas your JSON must satisfy: read ${AUTO}/schemas.ts (ITEM_SCHEMAS — one per item type).`,
  "",
  "ABILITY DESIGN — RARITY BUYS RULES, NOT STATS:",
  'To players, does this item feel full of depth or a single-strategy stat stick? For each ability ask "when will users have fun using this, and what is this ability for?"',
  "The ladder (enforced by the distinctiveness gate below):",
  "  common     — a solid tool with ONE spatial twist (a lunge, a recoil, a knockback that matters).",
  "  uncommon   — a small kit with spatial control (pulls, zones, wall-slams) or a passive.",
  "  rare       — internal set-up/payoff synergy: one ability creates a state, another has a rider that punishes it (riders: vs-status / execute / first-blood / near-wall).",
  "  epic       — changes a rule of play: blink moves, swap onHit, summons, energy conversion, charged (uses: N) ultimates.",
  "  legendary  — a build-around identity: combine a passive (aura / onKillEnergy / maxHp / regen) with abilities that reward playing to it. A player should reroute their whole kit for this.",
  "The verb toolbox beyond damage shapes: onHit pull/status/swap; riders; onKill refunds; recoil/lungeThrough/wallSlamDamage; move mode:'blink'; summon; convert; restore; uses charges; item passives (aura, onKillEnergy, maxHp, regen).",
  ...
  "NON-WEAPON TYPES: shields get 1-2 defensive abilities (+ optional passive); accessories are passive-first (rules, not stat sticks — at least one passive, at most one active); consumables get exactly ONE ability with uses (usually 1) — a per-encounter charge.",
  "AURA MAGNITUDES: an aura ticks at EVERY turn flip (twice per full round) — keep magnitude small (3-6).",
  "",
  "FIXING BALANCE:",
  ...
  "  1. Create/update one item — write its JSON to a temp file, then:",
  `       ${ENV} bun ${AUTO}/cli/upsert-item.ts ${dimId} d${dimId}-<name> <weapon|shield|accessory|consumable> < /path/to/item.json`,
  "     (type defaults to weapon; pass it explicitly for the other three.)",
  "  2. Run the item balance test (equips each item on a baseline hero vs dim-0 enemies; accessories/consumables ride on top of the base sword+shield kit; writes item-report-dim-" + dimId + ".json):",
  ...
  "  4. Run the distinctiveness gate (REQUIRED — it exits non-zero on failure and you must fix the roster, not bypass it). It fails on >40% plain damage-sticks, verb-identical duplicates within the roster, and epics/legendaries with no rule verb; it warns when an item is verb-identical to another dimension's item (fold a warned item's identity toward something the corpus lacks):",
  `       cd ${ROOT}/server && ${ENV} bun ${AUTO}/cli/roster-critic.ts ${dimId}`,
  "",
  "IMPORTANT: ability IDs must be globally unique kebab-case strings (prefix with the d" + dimId + "- item id).",
  "",
  "WORKFLOW:",
  "  1. Load the reference data above. 2. Create all items with upsert-item. 3. Run item-test. 4. Run item-rank to see rankings and outliers, and roster-critic for the distinctiveness gate. 5. Understand the cause, fix with upsert-item, re-test. Repeat until rankings track rarity, no item is worse-than-baseline, and roster-critic exits 0. Aim for ~3 test iterations.",
  "",
