...
describe("sword attack", () => {
  it("hits enemy in arc", () => {
    ...
    expect(resolveWeaponAttack(attacker, { x: 1, y: 0 }, entities, SHORT_SWORD_SLASH, emptyGrid)).toHaveLength(1);
  });
  ...
  it("misses enemy behind attacker", () => {
    ...
    const target = makeEntity("b", -40, 0, "blue");
    const entities = new Map([["a", attacker], ["b", target]]);
    expect(resolveWeaponAttack(attacker, { x: 1, y: 0 }, entities, SHORT_SWORD_SLASH, emptyGrid)).toHaveLength(0);
  });
...
describe("spear attack (rectangle)", () => {
  it("hits enemy in line", () => {
    ...
    const entities = new Map([["a", attacker], ["b", target]]);
    expect(resolveWeaponAttack(attacker, { x: 1, y: 0 }, entities, SPEAR_THRUST, emptyGrid)).toHaveLength(1);
  });
  ...
  it("misses enemy to the side", () => {
    ...
    const entities = new Map([["a", attacker], ["b", target]]);
    expect(resolveWeaponAttack(attacker, { x: 1, y: 0 }, entities, SPEAR_THRUST, emptyGrid)).toHaveLength(0);
  });
  ...
  it("hits multiple enemies in a line", () => {
    ...
    expect(resolveWeaponAttack(attacker, { x: 1, y: 0 }, entities, SPEAR_THRUST, emptyGrid)).toHaveLength(2);
  });
...
describe("bow attack (point/ray)", () => {
  it("hits closest enemy in line of fire", () => {
    ...
    const entities = new Map([["a", attacker], ["b", target]]);
    expect(resolveWeaponAttack(attacker, { x: 1, y: 0 }, entities, BOW_SHOT, emptyGrid)).toHaveLength(1);
  });
  ...
  it("misses enemy out of range", () => {
    ...
    const entities = new Map([["a", attacker], ["b", target]]);
    expect(resolveWeaponAttack(attacker, { x: 1, y: 0 }, entities, BOW_SHOT, emptyGrid)).toHaveLength(0);
  });
  ...
  it("only hits first enemy, not one behind", () => {
    ...
    const entities = new Map([["a", attacker], ["b", t1], ["c", t2]]);
    const hits = resolveWeaponAttack(attacker, { x: 1, y: 0 }, entities, BOW_SHOT, emptyGrid);
