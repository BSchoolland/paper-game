...
### Combat end
On `winner`: `combatEnd{won}` broadcast (animation gate). If `won && pendingHex` → `exploreHex(room, pendingHex)`, `phase="overworld"`, broadcast `hexMapState`. Else `resetToOrigin(room)` (new run). If party wiped on a run-ending condition → `gameOver`. Bump `generation`, tear down `session`.

### DB (R13)
`db.ts`: `DB_PATH = process.env.GAME_DB_PATH ?? "hex-discovery.sqlite"`. Migration at `user_version < 2`: drop/recreate `explored_hexes(run_id,q,r PK)`, `ALTER runs ADD dimension_id/created_at/host_token/active`. All exploration fns take `runId`: `saveExploredHex(runId,coord)`, `loadExploredHexes(runId)`, `clearExploredHexes(runId)`, `seedDiscovery(runId,radius)`. `startNewRun(dimensionId, hostToken?): number`. Module-load seed calls become an `initSeeds()` the harness can gate. No global-read caller remains.
